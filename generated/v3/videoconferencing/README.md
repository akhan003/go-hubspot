...
For using other server than the one defined on index 0 set context value `videoconferencing.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `videoconferencing.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `videoconferencing.ContextOperationServerIndices` and `videoconferencing.ContextOperationServerVariables` context maps.
...
auth := context.WithValue(
		context.Background(),
		videoconferencing.ContextAPIKeys,
		map[string]videoconferencing.APIKey{

...
For using other server than the one defined on index 0 set context value `crm_extensions.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `crm_extensions.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `crm_extensions.ContextOperationServerIndices` and `crm_extensions.ContextOperationServerVariables` context maps.
...
auth := context.WithValue(
		context.Background(),
		crm_extensions.ContextAPIKeys,
		map[string]crm_extensions.APIKey{
